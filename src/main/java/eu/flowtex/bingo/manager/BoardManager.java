...
public class BoardManager {
    private static HashMap<java.lang.Integer, String> defaults;
    ...
    public static void setCurrentScoreboard(final Player p, final String p2) {
        ...
        for (final Map.Entry<java.lang.Integer, String> integerStringEntry : BoardManager.defaults.entrySet()) {
            setLine(integerStringEntry.getKey(), integerStringEntry.getValue(), s);
        }
        p.setScoreboard(s);
    }

    public static void addLine(final java.lang.Integer s, final String s2) {
    ...
    static {
        BoardManager.defaults = new HashMap<java.lang.Integer, String>();
    }

    public static void lobbyScoreboard() {
        List<String> lines = Main.getInstance().getScoreboardConfiguration().getConfig().getStringList("scoreboard.lobby");
            ...
            if (Bukkit.getPlayer(uuid) != null) {
                ...
                if (teamitems.size() >= 5) {
                    ...
                    for (final Material teamitem : teamitems) {
                        BoardManager.setLine(x, "§8- §7" + ItemBuilder.getItemName(teamitem), player.getScoreboard());
                        if (--x <= 6) {
                            break;
                        }
