...
public class GameManager {
    ...
    public void startTeamState() {
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            String msg0 = Main.getInstance().getConfig().getString("general.gamename").replace("&", "§").replace("%colorcode%", colorcode);
            String msg1 = (Main.getInstance().getConfig().getString("lang.actionbarTeam").replace("&", "§").replace("%colorcode%", colorcode).replace("%gamename%", msg0));
        ...
        for (final UUID uuid : this.getTeamManager().getPlayersInTeam(Team.SPECTATOR)) {
            if (Bukkit.getPlayer(uuid) != null) {
                if (Bukkit.getPlayer(uuid).isOp()) {
                    ...
                    Bukkit.getPlayer(uuid).setGameMode(GameMode.SPECTATOR);
                    String msg0 = Main.getInstance().getConfig().getString("lang.noTeamOnStart").replace("&", "§");
    ...
    public void winEvent(final Team team) {
        ...
        String msg0 = Main.getInstance().getConfig().getString("general.gamename").replace("&", "§").replace("%colorcode%", colorcode);
        String msg1 = (Main.getInstance().getConfig().getString("lang.actionbarEnd").replace("&", "§").replace("%winTeam%", String.valueOf(team.getTeamid())).replace("%colorcode%", colorcode).replace("%gamename%", msg0));
