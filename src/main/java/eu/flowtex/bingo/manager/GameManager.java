...
public class GameManager {
    ...
    public void startTeamState() {
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            String msg0 = ChatColor.translateAlternateColorCodes('&', Main.getInstance().getDefaultConfiguration().getConfig().getString("general.gamename").replace("&", "§").replace("%colorcode%", colorcode));
            String msg1 = ChatColor.translateAlternateColorCodes('&', Main.getInstance().getMessageConfiguration().getConfig().getString("lang.actionbarTeam").replace("&", "§").replace("%colorcode%", colorcode).replace("%gamename%", msg0));
            ...
            if (Bukkit.getPlayer(uuid) != null) {
                if (Bukkit.getPlayer(uuid).isOp()) {
                    ...
                    if (Main.getInstance().getDefaultConfiguration().getConfig().getBoolean("general.removeTeamlessPlayersOnStart")) {
                        Bukkit.getPlayer(uuid).setGameMode(GameMode.SPECTATOR);
                        String msg0 = ChatColor.translateAlternateColorCodes('&', Main.getInstance().getMessageConfiguration().getConfig().getString("lang.noTeamOnStart"));
    ...
    public void winEvent(final Team team) {
        ...
        String msg1 = ChatColor.translateAlternateColorCodes('&', Main.getInstance().getDefaultConfiguration().getConfig().getString("lang.actionbarEnd").replace("%winTeam%", String.valueOf(team.getTeamid())).replace("%colorcode%", colorcode).replace("%gamename%", msg0));
